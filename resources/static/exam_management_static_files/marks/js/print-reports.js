/**
Generate PDF Templates for Certificates
**/
function generateCertificateTemplates(statisticsdata,studentdata,academicInfo,moderationList)
{
	console.log(statisticsdata)
	var margin;
	var imgData = "data:image/jpeg;base64,"
	var doc = new jsPDF();				
	doc.addImage(imgData, 'JPEG', 90, 40, 30,20)
	doc.setFontSize(8);
	//doc.setTextColor(255,0,0);
	//doc.text("Sample PDF Template for Certificate Printing(All data's are dummy).",  60, 10);	
	doc.setTextColor(0,0,0);
	doc.setFont("Lucida Handwriting");
	doc.setFontType("bolditalic");
	doc.setFontSize(16);
	doc.text("NATIONAL UNIVERSITY OF ADVANCED LEGAL STUDIES",  25, 25);							
	doc.setFontType("bold");
	doc.setFontSize(12);
	doc.text("Kochi - 683503",  90, 30);
	
	doc.setFontSize(12);
	doc.setFontType("italic");
	doc.text("Program : ",  30, 80);
	doc.text(academicInfo.program,  60, 80);
	
	doc.text("Batch : ",  30, 90);
	doc.text(academicInfo.batch,  60, 90);
	
	doc.text("Semester : ",  30, 100);
	doc.text(academicInfo.semester,  60, 100);
	
	doc.text("Exam : ",  30, 110);
	doc.text(academicInfo.examSchedule,  60, 110);
	
	doc.text("Min Internal Marks Required : ",  30, 120);
	doc.text(academicInfo.minInternal,  120, 120);
	
	doc.text("Min External Marks Required : ",  30, 130);
	doc.text(academicInfo.minExternal,  120, 130);
	
	doc.text("Min Total Marks Required : ",  30, 140);
	doc.text(academicInfo.minTotal,  120, 140);
	
	doc.setFontSize(12);
	doc.setFontType("bold");
	console.log(statisticsdata);
	doc.addPage('a4');
	var margin= {top:35,left:20}
	//if(statisticsdata!=undefined && statisticsdata.length>0){
		doc.text("Statistics Report for Moderation",  30, 20);	
		var head = [['Moderation Mark', 'No. of Students will Pass']]
		var rows = [];
		statisticsdata.forEach(element => { 
	   	 
	         temp = [element.shortage,element.studentcount];
	         rows.push(temp);
	   
	     });  
		
		doc.autoTable({ head: head, body: rows,margin: margin,  theme: 'grid',tableWidth: 120 })
	//}
	<!--Student Table -->
	doc.addPage('a3', 'landscape');
	
	doc.setFontSize(12);
	doc.setFontType("bold");
	//let table1Y = doc.lastAutoTable.finalY;
	//console.log(table1Y);
	doc.text("Failed Students List ",  20, 20);
	
	//var courses = studentdata[0];
	head = []
	var subHeading=["Reg No","Name"];
	var headItem1 =	   {
	          content: "Student",  	          
	          colSpan:2,
	          styles: { halign: 'center',fillColor: [22, 160, 135]}	          
	        }
	 
	
	var courseTitles=[headItem1];
	var courseNames = getCourseNamesForModeration(studentdata);//[];
	//courses.markdetails.forEach(element => { 
	courseNames.forEach(element=>{
	    console.log(element);	   
	    var headItem =	   {
	          //content: element.coursename,
	    	  content: element,
	          colSpan: 3,	       
	          styles: { halign: 'center',fillColor: [22, 160, 135]}	          
	        }
	     //courseNames.push(element.coursename);
	     subHeading.push("Internal");
	     subHeading.push("External");
	     //if(statisticsdata==undefined)
	     //{subHeading.push("Mod");}
	     subHeading.push("P/F (Shortage)");
		 courseTitles.push(headItem);		
	});  	
	
	head.push(courseTitles);
	head.push(subHeading);
	
	
	var marklist=[];
	
	console.log(courseNames);
	studentdata.forEach(element => { 
		var temp= [element.studentid,element.studentname]
		//if(element.markdetails.length>0){
	    courseNames.forEach(course=>{
	    	console.log(course);
	    	
	    		//temp = [element.studentid,element.studentname]
	    		var course_mark = element.markdetails.find(mark=>mark.coursename===course);
		    	console.log(course_mark);
		    	if(course_mark!=undefined){
			    	temp.push(course_mark.internal);
					temp.push(course_mark.external);
				//	if(statisticsdata==undefined ){
					//temp.push(course_mark.shortage);
				//	}
					//temp.push(course_mark.total);
					if(course_mark.total>=academicInfo.minTotal){
						temp.push("P ");
					}else if(course_mark.shortage>0)
						{
						temp.push("F ("+course_mark.shortage+")");
						}
					else
						{
						temp.push("P ");
						}
						
		    	}else
		    		{
			    		temp.push('-');
			    		temp.push('-');
			    		temp.push('-');
		    		}
	    	
	    	
	    	
	    })
	   // }
		//if(temp.length>0)
		marklist.push(temp);
	})
	
	doc.autoTable({ head: head, body: marklist,margin: margin,  theme: 'grid',rowPageBreak: 'avoid',tableWidth: 'auto',
	     columnWidth: 'wrap', styles: {
	         fontSize: 9,
	         cellWidth: 'wrap'
	       }})
	       
	 <!--Moderation Table -->
	
	if(moderationList!=undefined ){	
	
	
	doc.addPage('a3', 'landscape');
	
	doc.setFontSize(12);
	doc.setFontType("bold");
	//let table1Y = doc.lastAutoTable.finalY;
	//console.log(table1Y);
	doc.text("Moderation Sheet ",  20, 20);
	doc.text("Moderation Mark : "+academicInfo.moderationMark,20,30);
	//var courses = studentdata[0];
	head = []
	subHeading=["Reg No","Name"];
	headItem1 =	   {
	          content: "Student",  	          
	          colSpan:2,
	          styles: { halign: 'center',fillColor: [22, 160, 135]}	          
	        }
	 
	
	courseTitles=[headItem1];
	courseNames = getCourseNamesForModeration(moderationList);//[];
	//courses.markdetails.forEach(element => { 
	courseNames.forEach(element=>{
	    console.log(element);	   
	    var headItem =	   {
	          //content: element.coursename,
	    	  content: element,
	          colSpan: 3,	       
	          styles: { halign: 'center',fillColor: [22, 160, 135]}	          
	        }
	     //courseNames.push(element.coursename);
	     subHeading.push("Internal");
	     subHeading.push("External");	    
	     subHeading.push("Mod");
	     subHeading.push("P/F");
		 courseTitles.push(headItem);		
	});  	
	
	head.push(courseTitles);
	head.push(subHeading);
	
	
	var marklist=[];
	
	console.log(courseNames);
	moderationList.forEach(element => { 
		var temp =  [element.studentid,element.studentname]
		//if(element.markdetails.length>0){
	    courseNames.forEach(course=>{
	    	console.log(course);
	    	
	    		//temp = [element.studentid,element.studentname]
	    		var course_mark = element.markdetails.find(mark=>mark.coursename===course);
		    	console.log(course_mark);
		    	if(course_mark!=undefined){
			    	temp.push(course_mark.internal);
					temp.push(course_mark.external);
					temp.push(course_mark.shortage);				
					if(course_mark.total>=academicInfo.minTotal){
						temp.push("P");
					}else if(course_mark.shortage>0)
						{
						temp.push("F");
						}
					else
						{
						temp.push("P");
						}
						
		    	}else
		    		{
			    		temp.push('-');
			    		temp.push('-');
			    		temp.push('-');
		    		}
	    	
	    	
	    	
	    })
	   // }
		if(temp.length>0)
		marklist.push(temp);
	})
	
	doc.autoTable({ head: head, body: marklist,margin: margin,  theme: 'grid',rowPageBreak: 'avoid',tableWidth: 'auto',
	     columnWidth: 'wrap', styles: {
	         fontSize: 9,
	         cellWidth: 'wrap'
	       }})
	          
	       
	       
	
		var iframe = document.getElementById('moderationFrame');
		iframe.src = doc.output('datauristring');
		
	}else
		{
		window.open(doc.output('bloburl'), '_blank');	
		}
	
	
}

function getCourseNamesForModeration(studentData)
{
	var courseName = [];
	studentData.forEach(student=>{
		student.markdetails.forEach(mark=>{
			if(!courseName.includes(mark.coursename))
				{
				courseName.push(mark.coursename)
				}
		})
	})
	console.log(courseName);
	return courseName;
}