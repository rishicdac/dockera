...
**/
function generateFinalMarkSheet(markdata)
{
		...
		doc.setFontSize(16);
		text = "NATIONAL UNIVERSITY OF ADVANCED LEGAL STUDIES";
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		doc.text(text, 25, yPos);
		
		doc.setFontType("bold");
		doc.setFontSize(12);
		text = "Kalamassery, Kochi - 683503, Kerala, India"
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		...
		doc.text("No:"+markdata.slNumber, 25, yPos);
		doc.text(text, 150, yPos);
		
		doc.setFontSize(12);
		text="The following is the statement of marks awarded to Sri./Smt "+ markdata.studentName +" at the " +markdata.semester;
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight+10
		doc.text(text, 25, yPos);
		
		text= " examination of the "+markdata.programName +" Degree Course held in "+ markdata.monthYear+"." ;
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight+3
		doc.text(text, 25, yPos);
		
		doc.setFontType("bold");
		text= "Register Number: "+markdata.regNumber;
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight
		doc.text(text, 130, yPos+10);
		
		var title = [['Course Code', 'Course Title','External(Max 50)','Internal (Max 50)','Total(Max 100)','Credits','Grade']]
		var content = getMarks(markdata.marks);
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+20
		doc.autoTable({ head: title, body: content,margin: {top: yPos,left:20},  theme: 'grid',tableWidth: 180 })
		
		doc.setFontSize(12);
		doc.setFontType("normal");
		text="Total credits : "+markdata.totalCredit;
		lineheight = doc.lastAutoTable.finalY;
		yPos = lineheight+10
		doc.text(text, 85, yPos);
		doc.text("Core : "+markdata.totalCore, 120, yPos);
		doc.text("Elective : "+markdata.totalElective, 150, yPos);
		
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight+5
		text="Min Credits Required : "+markdata.minCredits;
		doc.text(text,70, yPos);
		doc.text("Core : "+markdata.minCore, 120, yPos);
		doc.text("Elective : "+markdata.minElective, 150, yPos);
		
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight+5
		text="Total credits acquired in all the previous semesters : "+markdata.prevCredits;		
		doc.text(text, 20, yPos);
		doc.text("Core : "+markdata.prevCore, 120, yPos);
		doc.text("Elective : "+markdata.prevElective, 150, yPos);
		
		doc.setFontType("bold");
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight+5
		text="GPA : "+markdata.gpa;		
		doc.text(text, 90, yPos);
		
		doc.setFontType("bold");
		text= "Marks Entered By: ";
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight+50
		doc.text(text, 25, yPos);
		
		doc.setFontType("bold");
		text= "Marks Checked By: ";
		lineheight = doc.getLineHeight(text) / doc.internal.scaleFactor;
		yPos = yPos+lineheight
		doc.text(text, 25, yPos);
	...
	/*	doc.addPage();
		...
		//Grade Details Table 
		title = [['Course Code', 'Course Title','External(Max 50)','Internal (Max 50)','Total(Max 100)','Credits','Grade']]
		doc.autoTable({ head: title, body: content,margin: {top: yPos,left:20},  theme: 'grid',tableWidth: 180 })
...
function getMarks(marks)
{
	...
	marks.forEach(mark=>{
		var mdata = [];
		mdata.push(mark.courseCode);
		mdata.push(mark.courseName);
		mdata.push(mark.external);
		mdata.push(mark.internal);
		mdata.push(mark.total);
		mdata.push(mark.credits);
