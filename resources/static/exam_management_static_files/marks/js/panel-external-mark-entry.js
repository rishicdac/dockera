...
{	
			var  examName =$("#selectExamType option:selected").text();
			var res = examName.split("-");
            var examType = res[res.length-1];
//            alert(examType);
            getCoursesOfPanel(examType);
});
/**
 * Get User Panel
 * 
...
function getUserPanels()
{
	...
	$.ajax({
   		url:PANEL_MANAGEMENT+'getApprovedExamPanelsOfUser',
   		method:'GET',   
   		async:false,
   		success:function(result)
   		{   			
   			setUpDropDown("selectPanel",result.data,"courseCode","courseName");
   			$('#selectPanel').unbind().bind('change',function()
   			{
   			
   				getCoursesOfPanel(panelId,userCode);
...
/**
 * Load courses under panel
 * 
 */
function getCoursesOfPanel(examType)
{
	var data={"batchId":$('#selectBatch').val(),"category":examType,"programId": $('#selectProgram').val(),"semester":$('#selectSemester').val()};
	//var data={"batchId":1261,"category":"Regular","programId": 491,"semester":"semester2"};

	
	$.ajax({
   		url:PANEL_MANAGEMENT+'getExamPanelCourseAllocations',
   		method:'GET',
   		data:data,
   		async:false,
   		success:function(result)
   		{   			
   			if(result.length == 0)
   				toastr.error("No Courses alloted for the selected exam. Kindly contact admin.");
   			else		
     			setUpDropDown("selectPanelCourse",result,"courseCode","courseName");
...
function fetchStudentList()
{
    ...
    $('#markForm').submit(function(e){
		   ...
		   var courseId = $('#selectPanelCourse').val();
...
$("#externalMarksTbl tbody").on('click', "#saveIcon", function(e) {
    ...
    $.each($tds, function(i, el) {
      ...
      switch(i)
      	{
      	case 0:	      	
      		$('#externalMarksTbl').dataTable().fnUpdate(parseInt(txt), [$row], $(this), false);  
...
function createMarkJSON(tableData,approvalids)
{
	 ...
	 externalMark.courseName =  $("#selectPanelCourse option:selected").text();
...
function createAcademicInfo()
{
	...
	academicInfo.courseCode = $('#selectPanelCourse').val();
...
function checkFacultyMarkEntered()
{
	...
	var formData =  {
			  ...
			  "courseCode":$('#selectPanelCourse').val(),
