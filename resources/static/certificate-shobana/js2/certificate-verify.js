...
function loadDatatable(data){
	var verificationDataTable =  $('#certificateVerificationTbl').DataTable({
         "language": {
                    "emptyTable": "No data available for the verification"
                  },
			//"processing": true,
			"destroy": true,
			//"ajax": "certificate-management/api/v1/academics/certificate-management/getAllCertificateRequests/SUBMITTED",
			//"ajax":"http://127.0.0.1:6004/api/v1/academics/certificate-management/getAllCertificateDetails",
			/*"ajax":{ 
			   "url":  BASE_URL +"getAllCertificateRequests/SUBMITTED",
		...
		$('#certificateVerificationTbl tbody').on('click', '#detailsButton', function () {
			var data = verificationDataTable.row($('#certificateVerificationTbl tbody').parents('tr')).data();
			$('#verifyBtn').hide();
			$('#feeStatusText').hide();
