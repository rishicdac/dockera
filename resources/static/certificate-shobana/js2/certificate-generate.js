$( document ).ready(function() { 
	...
	var certificateId,actionType,nameId,printCount;

	/**
	 Data table load
	**/
		var generateDataTable =  $('#certificateGenerateTbl').DataTable({
			 "language": {
		...
		$('#certificateGenerateTbl tbody').on('click', '#print', function () {
			var msg  = {
					...
					ConfirmDialog(msg,"SAVE").then( function(response){
						...
						var data = generateDataTable.row($('#certificateGenerateTbl tbody').parents('tr')).data();
		...
		**/
		function printCertificate()
		{
			var data={};
			console.log(data);
			data.certificateNumber="NUALS/015/2020";//TODO add certificate Number
			data.grade="Second Class";//TODO add student class details
			data.course="POST GRADUATE DIPLOMA IN CYBER LAW(PGDCL)";//TODO add student course
			data.studentName="Thomas Roy";//TODO Student name
		
			
			var statusdata = {"certificatesId":certificateId,"remarks":$("#certRemarks").val()}
			...
			$.ajax({
			   ...
			   {				
				 generateCertificateTemplates(data,nameId);
		...
		**/
		function generateCertificateTemplates(data,certificateTypeId)
		{
			...
			switch (certificateTypeId) {
				case DEGREE://Degree Certificate
				    doc.setFontSize(10);
					doc.text("data.certificateNumber",  150, 45);
					...
					doc.setFontSize(12);
					doc.setFontType("italic");
					doc.text("is being conferred on",  90, 150);
					doc.setFontSize(14);
					doc.setFontType("bolditalic");
					doc.text("Anupama", 90, 160);
					doc.setFontSize(12);
					doc.setFontType("italic");
					...
					break;
				case PROVISIONAL://Provisional certificate
					doc.setFontSize(10);
					...
					doc.setFontSize(12);
					doc.setFontType("italic");
					doc.text("Certified that NIMMY JOSE candidate with Register No: LB0012345 at the ",  40, 110);
					doc.text("Five Year B.A LLB(Hons) Degree Examination held in JUNE 2020 has passed",  40, 120);
					...
					window.open(doc.output('bloburl'), '_blank');
					break;
				case TRANSCRIPT://Transcript certificate
				  toastr.error(" No template provided for transcript");
					break;
				case MIGRATION://Migration certificate
					doc.setFontSize(10);
					...
					doc.setFontSize(12);
					doc.setFontType("italic");
					doc.text("This is to certify that Mr./Ms. BINCY JAMES GEORGE (Roll No. 11706) was a ",  40, 110);
					doc.text("bonfide student of this University for the 5 Year B A LLB(Hons) Degree Course ",  40, 120);
					...
					window.open(doc.output('bloburl'), '_blank');
					break;
				case TRANSFER://Transfer certificate
					doc.setFontSize(10);
					doc.text("No.NUALS/AC (B)/03/",  130, 45);
					doc.text("Roll No.LL.B/MIG",  130, 50);
					doc.text("Date :"+  moment().format('LL'),  130, 55);
					...
					window.open(doc.output('bloburl'), '_blank');
					break;
				case CONDUCT://Conduct certificate
				    doc.setFontSize(10);
					doc.text("No. NUALS/AC(B)/02/1128/LLB/C&C/20",  130, 45);
					doc.text("Date :"+  moment().format('LL'),  130, 55);
					doc.setFontSize(12);
					doc.setFontType("bold");
					...
					window.open(doc.output('bloburl'), '_blank');
					break;
				case ATTENDENCE://Attendance certificate
					break;
				case CONSOLIDATED_MARK_SHEET://Consolidated Marklist
				   toastr.error(" No template provided for transcript");
					break;
				case NO_DUES://No Due Certificate
					doc.setFontSize(10);
					doc.text("No.NUALS/AC (B)/03/",  130, 45);
					doc.text("Roll No.LL.B/MIG",  130, 50);
					doc.text("Date :"+  moment().format('LL'),  130, 55);
